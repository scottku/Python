def list_comprehension():
    """ 리스트 내포
        Syntax: [ 표현식 fpr 변수 in 수치형 if 조건식] """
    # 1~10까지의 리스트가 있을 때, 내부 항목을 전부 제곱한 새로운 리스트 생성
    ...
    # strings의 요소 중에서 문자열의 길이가 3 이하인 문자열 리스트 생성
    results = []
    for item in strings:
        if len(item) <= 3:
            results.append(item)
...
def set_comprehension():
    ...
    strings = ['a', 'as', 'bat', 'car', 'dove', 'python']
    # strings의 요소 중에서 문자열의 길이가 3 이하인 SET 생성
    results = { item for item in strings if len(item) <= 3}
    print(results)

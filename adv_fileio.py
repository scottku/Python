...
def copy_binary():
    ...
    f_src = open("./sample/고루시콘1.jpeg", "rb") # binary 모드로 설정
    data = f_src.read()
    print(type(data))
    f_src.close()

    f_dest = open("./sample/고루시콘1-copy.jpeg", "wb")
    f_dest.write(data)
    f_dest.close()

def pickle_dump():
    with open("./sample/players.bin", "wb") as f: # 자원 자동 정리, pickle 사용을 위해 binary 모드를 이용
        data = {'baseball': 9}
        pickle.dump(data, f)
    print("덤프 완료!")

def pickle_load(): # 객체 역직렬화: 2진 데이터 -> 파이썬 객체로 복원
...
def pickle_dump_multi():
    # dump를 중목 실행하면 여러 객체 dump 가능
    with open("./sample/players.bin", "wb") as f:
        pickle.dump({"baseball": 9}, f, 1) # 프로토콜의 버전 명시 가능(ex: 1)
        pickle.dump({"basketball": 5}, f, pickle.HIGHEST_PROTOCOL) # 가장 최신 버전의 프로토콜
        pickle.dump({"soccer": 11}, f) # 프로토콜 버전을 명시하지 않으면 가장 최신 버전의 프로토콜 사용

    print("중복 덤프 완료!")

def pickle_load_multi():
    # 파일 내부에 몇 개의 피클 객체가 저장되어 있는지 확인이 어려움
    with open("./sample/players.bin", "rb") as f:
        # print(pickle.load(f))
        # print(pickle.load(f))
        # print(pickle.load(f))
        # print(pickle.load(f)) # End Of File Error 발생 (더 이상 읽을 객체 없음)
        data_list = []
        while True:
            try:
                data = pickle.load(f)
